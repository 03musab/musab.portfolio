...
const DriftWall = ({
  ...
  const columnItems = useMemo(() => {
    // Each column cycles through ALL items, rotated by its column index, so
    // every row of the wall shows a different set of images (no repeats).
    return Array.from({ length: columns }, (_, c) =>
      items.map((_, i) => items[(i + c) % items.length])
    );
  }, [items, columns]);
